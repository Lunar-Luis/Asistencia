...
// ==========================================
// COMPONENTES SKELETON (Restaurados)
// ==========================================
const SkeletonStats = () => (
  <motion.div {...animProps} className={`${cardStyle} !p-6 animate-pulse flex flex-col justify-between`}>
    <div className="flex justify-between items-start">
      <div className="w-12 h-12 rounded-2xl bg-slate-200 dark:bg-slate-800"></div>
      <div className="w-12 h-3 rounded-full bg-slate-200 dark:bg-slate-800"></div>
    </div>
    <div className="mt-5">
      <div className="w-20 h-8 bg-slate-200 dark:bg-slate-800 rounded mb-2"></div>
...
const SkeletonActivity = () => (
  <motion.div {...animProps} className={`${cardStyle} animate-pulse mb-8`}>
    <div className="flex justify-between items-end mb-8">
      ...
      <div className="w-32 h-8 bg-slate-200 dark:bg-slate-800 rounded-xl hidden sm:block"></div>
    </div>
...
const SkeletonChart = () => (
  <motion.div {...animProps} className={`${cardStyle} relative animate-pulse h-full flex flex-col min-h-[350px]`}>
    <div className="flex flex-col sm:flex-row justify-between items-start mb-10 gap-4">
      ...
      <div className="w-full sm:w-48 h-10 bg-slate-200 dark:bg-slate-800 rounded-2xl"></div>
    </div>
    ...
    <div className="grid grid-cols-2 gap-4">
      {[1,2,3,4,5,6].map(i => (
        <div key={i} className="flex flex-col items-center p-4 sm:p-5 rounded-3xl bg-slate-50 dark:bg-slate-800/50">
          <div className="w-12 h-12 rounded-2xl bg-slate-200 dark:bg-slate-800 mb-3"></div>
...
export default function Inicio() {
  ...
  const stats = [
    { title: "Total Empleados", value: "142", icon: <Users size={24} />, color: "text-primary", bg: "bg-primary/10", trend: "+12%" },
    { title: "Presentes", value: "118", icon: <CheckCircle2 size={24} />, color: "text-success", bg: "bg-success/10", trend: "80%" },
    { title: "Tarde", value: "8", icon: <Clock size={24} />, color: "text-warning", bg: "bg-warning/10", trend: "15%" },
    { title: "Ausentes", value: "12", icon: <XCircle size={24} />, color: "text-danger", bg: "bg-danger/10", trend: "5%" },
  ];

  const chartData = [
    { fecha: "10 Mar", p: 110, t: 15, a: 17 }, { fecha: "11 Mar", p: 125, t: 8, a: 9 },
    { fecha: "12 Mar", p: 118, t: 12, a: 12 }, { fecha: "13 Mar", p: 130, t: 5, a: 7 },
    { fecha: "14 Mar", p: 115, t: 10, a: 17 }, { fecha: "15 Mar", p: 135, t: 2, a: 5 },
            ...
            {stats.map((stat, i) => (
              <motion.div key={i} {...animProps} transition={{ delay: i * 0.05 }} className={`${cardStyle} !p-6 cursor-default flex flex-col justify-between`}>
                <div className="flex justify-between items-start">
                  ...
                  <span className={`text-xs font-black flex items-center gap-1 ${stat.color} tracking-widest`}>
                    <TrendingUp size={16} /> {stat.trend}
                  </span>
                </div>
                <div className="mt-5">
                  {/* TAMAÑO DE NÚMEROS REDUCIDO (text-3xl sm:text-4xl en vez de 5xl) */}
        ...
        <motion.div {...animProps} transition={{ delay: 0.15 }} className={`${cardStyle} mb-8`}>
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-8 pb-6 border-b border-slate-100 dark:border-slate-800">
            <div>
              ...
              <p className="text-[11px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">Últimos registros nfc detectados</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {[
              { name: "Sebastián López", time: "08:00 AM", status: "A tiempo", icon: <CheckCircle2 size={14}/>, img: "SL", color: "text-emerald-600 dark:text-success", bg: "bg-emerald-50 dark:bg-success/10" },
              { name: "Andrea Pérez", time: "08:15 AM", status: "Tarde", icon: <AlertCircle size={14}/>, img: "AP", color: "text-amber-600 dark:text-warning", bg: "bg-amber-50 dark:bg-warning/10" },
              { name: "Juan Castro", time: "---", status: "Ausente", icon: <XCircle size={14}/>, img: "JC", color: "text-red-500 dark:text-danger", bg: "bg-red-50 dark:bg-danger/10" },
              { name: "Anurbe Dev", time: "07:55 AM", status: "A tiempo", icon: <CheckCircle2 size={14}/>, img: "AD", color: "text-emerald-600 dark:text-success", bg: "bg-emerald-50 dark:bg-success/10" },
            ].map((user, i) => (
              <div key={i} className="flex items-center gap-4 p-4 sm:p-5 rounded-[2rem] bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 hover:border-slate-200 dark:hover:border-slate-700 transition-colors">
                <div className="w-14 h-14 rounded-xl bg-primary/10 flex items-center justify-center text-sm font-black text-primary uppercase shrink-0">{user.img}</div>
                <div className="flex-1 min-w-0">
                  <p className="text-[13px] sm:text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight truncate">{user.name}</p>
                  <p className="text-[11px] sm:text-xs text-primary font-bold uppercase italic mt-0.5">{user.time}</p>
                </div>
                <span className={`inline-flex items-center gap-1 px-3 py-1.5 rounded-lg text-[10px] font-black uppercase tracking-wider shrink-0 ${user.bg} ${user.color}`}>
            ...
            {isLoading ? (
              ...
              <motion.div {...animProps} transition={{ delay: 0.2 }} className={`${cardStyle} relative h-full flex flex-col min-h-[350px]`}>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-8 pb-6 border-b border-slate-100 dark:border-slate-800 gap-4">
                  <div>
                    <h3 className="font-black uppercase italic tracking-tighter dark:text-white text-2xl text-slate-800">Resumen Semanal</h3>
                    <p className="text-[11px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">Monitoreo interactivo de asistencia</p>
                  </div>
                  <div className="flex flex-wrap gap-3 sm:gap-4 items-center bg-slate-50 dark:bg-slate-800/50 p-3 px-4 sm:px-5 rounded-2xl w-full sm:w-auto">
                    <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-success"></span><span className="text-[10px] sm:text-[11px] font-black dark:text-white uppercase tracking-widest">Presente</span></div>
                    <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-warning"></span><span className="text-[10px] sm:text-[11px] font-black dark:text-white uppercase tracking-widest">Tarde</span></div>
                    <div className="flex items-center gap-2"><span className="w-2.5 h-2.5 rounded-full bg-danger"></span><span className="text-[10px] sm:text-[11px] font-black dark:text-white uppercase tracking-widest">Ausente</span></div>
                  </div>
                </div>
                  ...
                  <AnimatePresence>
                    {hoveredValue && (
                      <motion.div initial={{ opacity: 0, scale: 0.8, y: 10 }} animate={{ opacity: 1, scale: 1, y: 0 }} exit={{ opacity: 0, scale: 0.8 }}
                        className="absolute z-50 bg-slate-800 dark:bg-white text-white dark:text-slate-900 text-[11px] sm:text-xs font-black px-4 sm:px-5 py-2.5 sm:py-3.5 rounded-2xl shadow-xl pointer-events-none uppercase italic flex flex-col items-center"
                        style={{ left: `${hoveredValue.x}%`, bottom: `${hoveredValue.y + 10}%`, transform: 'translateX(-50%)' }}
                      >
                        <span className="opacity-70 text-[9px] sm:text-[10px] tracking-widest mb-0.5">{hoveredValue.label}</span>
                        <span>{hoveredValue.val} EMPLEADOS</span>
                        <div className="w-3 h-3 bg-slate-800 dark:bg-white rotate-45 absolute -bottom-1"></div>
                      </motion.div>
                    )}
                  </AnimatePresence>
                    ...
                    return (
                      <div key={i} className="w-[11%] h-full flex flex-col justify-end items-center gap-2 sm:gap-4 group">
                        <div className="w-full flex flex-col-reverse h-full bg-slate-50 dark:bg-slate-800/50 rounded-xl sm:rounded-2xl relative overflow-hidden transition-colors">
                            <motion.div initial={{height:0}} animate={{height: `${pHeight}%`}} className="bg-success w-full relative cursor-pointer hover:brightness-110 transition-all border-t border-white/10 dark:border-black/10" onMouseEnter={() => setHoveredValue({ val: data.p, label: "Presentes", x: (i * 14.3) + 6, y: pHeight })} onMouseLeave={() => setHoveredValue(null)} />
                            <motion.div initial={{height:0}} animate={{height: `${tHeight}%`}} className="bg-warning w-full relative cursor-pointer hover:brightness-110 transition-all border-t border-white/10 dark:border-black/10" onMouseEnter={() => setHoveredValue({ val: data.t, label: "Tarde", x: (i * 14.3) + 6, y: pHeight + tHeight })} onMouseLeave={() => setHoveredValue(null)} />
                            <motion.div initial={{height:0}} animate={{height: `${aHeight}%`}} className="bg-danger w-full relative cursor-pointer hover:brightness-110 transition-all" onMouseEnter={() => setHoveredValue({ val: data.a, label: "Ausentes", x: (i * 14.3) + 6, y: pHeight + tHeight + aHeight })} onMouseLeave={() => setHoveredValue(null)} />
                        </div>
                        <span className="text-[9px] sm:text-[11px] font-black text-slate-400 dark:text-slate-500 uppercase tracking-widest">{data.fecha}</span>
                      </div>
                    );
                  })}
                </div>
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    <Link key={i} to={action.path} className={`flex flex-col items-center p-4 sm:p-5 rounded-3xl bg-slate-50 dark:bg-slate-800/50 ${actionHoverEffect}`}>
                      <div className={`p-3.5 sm:p-4 rounded-2xl ${action.color} text-white shadow-sm mb-3 sm:mb-4`}>
                        {action.icon}
                      </div>
