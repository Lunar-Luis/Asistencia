...
import type { Asistencia, DashboardResumen } from '../services/api';
...
export default function Inicio() {
  ...
  const [dashboardStats, setDashboardStats] = useState<DashboardResumen | null>(null);
  ...
  const hoyDisplay = new Date().toLocaleDateString('es-VE', { year: 'numeric', month: 'short', day: 'numeric' });
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setActividadReciente(actividadHoy);
      } catch (error) {
  ...
  const stats = dashboardStats ? [
    { title: "Total Empleados", value: dashboardStats.totalEmpleados, icon: <Users size={24} />, color: "text-primary", bg: "bg-primary/10", trend: "Activos" },
    { title: "Presentes Hoy", value: dashboardStats.presentesHoy, icon: <CheckCircle2 size={24} />, color: "text-emerald-500 dark:text-success", bg: "bg-emerald-50 dark:bg-success/10", trend: "Al día" },
    { title: "Llegadas Tarde", value: dashboardStats.tardeHoy, icon: <Clock size={24} />, color: "text-amber-500 dark:text-warning", bg: "bg-amber-50 dark:bg-warning/10", trend: "Atención" },
    { title: "Ausentes Hoy", value: dashboardStats.ausentesHoy, icon: <XCircle size={24} />, color: "text-red-500 dark:text-danger", bg: "bg-red-50 dark:bg-danger/10", trend: "Revisar" },
  ] : [];

  const chartData = dashboardStats?.chartData || [];
  const maxEmpleados = dashboardStats?.totalEmpleados || 10;

  const formatTime = (dateString: string) => {
    return new Date(dateString).toLocaleTimeString('es-VE', { hour: '2-digit', minute: '2-digit', hour12: true });
  };
    ...
    <main className="pb-8 max-w-[1600px] mx-auto">
      ...
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="mb-6">
        ...
        <p className="text-[11px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-[0.3em] sm:tracking-[0.4em] mt-1 leading-relaxed">
          Resumen Global de Asistencia <br className="block" />{hoyDisplay}
        </p>
      </motion.div>
        ...
        <motion.div {...animProps} transition={{ delay: 0.15 }} className={`${cardStyle} mb-8`}>
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4 mb-8 pb-6 border-b border-slate-100 dark:border-slate-800">
            <div>
              ...
              <p className="text-[11px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">Registros de hoy</p>
            </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
            {actividadReciente.length > 0 ? actividadReciente.map((asistencia) => {
              const iniciales = `${asistencia.empleado.nombre.charAt(0)}${asistencia.empleado.apellido.charAt(0)}`.toUpperCase();
              const esSalida = asistencia.marcaSalida !== null;
              
              return (
                <div key={asistencia.id} className="flex items-center gap-4 p-4 sm:p-5 rounded-[2rem] bg-slate-50 dark:bg-slate-800/50 border border-slate-100 dark:border-slate-800 hover:border-slate-200 dark:hover:border-slate-700 transition-colors">
                  <div className="w-14 h-14 rounded-xl bg-primary/10 flex items-center justify-center text-sm font-black text-primary uppercase shrink-0">{iniciales}</div>
                  <div className="flex-1 min-w-0">
                    <p className="text-[13px] sm:text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight truncate">
                      {asistencia.empleado.nombre} {asistencia.empleado.apellido.split(' ')[0]}
                    </p>
                    <p className="text-[11px] sm:text-xs text-primary font-bold uppercase italic mt-0.5">
              ...
              <p className="text-slate-500 text-sm font-bold p-4 col-span-full text-center">No hay actividad reciente hoy.</p>
            )}
          </div>
        </motion.div>
      )}
              ...
              <motion.div {...animProps} transition={{ delay: 0.2 }} className={`${cardStyle} relative h-full flex flex-col min-h-[420px]`}>
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-10 gap-4">
                  <div>
                    ...
                    <p className="text-[11px] sm:text-xs font-bold text-slate-500 dark:text-slate-400 uppercase tracking-widest mt-1">Asistencia de los últimos 5 días</p>
                  </div>
                ...
                <div className="relative flex-1 w-full flex items-end justify-between mt-4 pb-8 border-b border-slate-100 dark:border-slate-800/50">
                  <div className="absolute inset-0 flex flex-col justify-between pointer-events-none pb-8">
                    {[100, 75, 50, 25, 0].map(p => (
                      <div key={p} className="w-full border-t border-dashed border-slate-200 dark:border-slate-800/80 flex items-center">
                        <span className="absolute -left-1 sm:-left-4 -translate-x-full text-[9px] font-bold text-slate-400">
                          {maxEmpleados === 0 ? 0 : Math.round((p/100)*maxEmpleados)}
                        </span>
                      </div>
                    ))}
                  </div>
                  ...
                  {chartData.map((data, i) => {
                    ...
                    return (
                      <div 
                        key={i} 
                        className="relative w-[15%] sm:w-[12%] h-full flex flex-col justify-end items-center group cursor-pointer z-10"
