...
const SkeletonCard = () => (
  <motion.div {...animProps} className={`${cardStyle} animate-pulse flex flex-col h-full`}>
    ...
    <div className="grid grid-cols-2 gap-4 py-3 border-t border-slate-50 dark:border-slate-800/50 mb-4">
       <div>
         <div className="h-2 bg-slate-200 dark:bg-slate-800 rounded w-1/2 mb-2"></div>
...
export default function Empleados() {
  ...
  const [empleados, setEmpleados] = useState([
    { id: 1, nombre: 'Anurbe Martínez', cedula: '25.123.456', cargo: 'Administrador', departamento: 'Tecnología', correo: 'anurbe@empresa.com', telefono: '+1 234 567 8901', nfc_uid: 'A1:B2:C3:D4', ingreso: '12/01/2024', horario: 'Horario Normal', status: 'Activo', foto: '/images/ANURBE.jpg' },
    { id: 2, nombre: 'Andrea Torres', cedula: '24.987.654', cargo: 'Recursos Humanos', departamento: 'Talento', correo: 'andrea@empresa.com', telefono: '+1 987 654 3210', nfc_uid: 'D4:C3:B2:A1', ingreso: '05/03/2023', horario: 'Flexible', status: 'Inactivo', foto: null },
  ]);
  ...
  const handleImageChange = (e: React.ChangeEvent<HTMLInputElement>) => {
    if (e.target.files && e.target.files[0]) { setSelectedImage(URL.createObjectURL(e.target.files[0])); }
  };
            ...
            <AnimatePresence mode="popLayout">
              {filteredEmpleados.map((emp, index) => (
                <motion.div
                  ...
                  className={`${cardStyle} flex flex-col group ${actionHoverEffect} ${emp.status === 'Inactivo' ? 'opacity-75 hover:opacity-100 grayscale-[0.3]' : ''}`}
                >
                  ...
                  {/* Ajustado: mb-4 en lugar de mb-6 */}
                  <div className="flex items-center gap-4 mb-4">
                    ...
                    {/* Ajustado: pr-10 en lugar de pr-12 */}
                  ...
                  {/* Ajustado: space-y-2 en lugar de space-y-3 */}
                  ...
                  {/* Ajustado: py-3 mb-3 en lugar de py-4 mb-5 */}
                  ...
                  {/* Ajustado: mt-1.5 en lugar de mt-auto */}
              ...
              <form className="grid grid-cols-1 md:grid-cols-2 gap-6" onSubmit={(e) => { e.preventDefault(); setIsModalOpen(false); }}>
                <div className="flex flex-col items-center justify-center border-b md:border-b-0 md:border-r border-slate-100 dark:border-slate-800 pb-6 md:pb-0 md:pr-6 space-y-6">
                  <div className="w-32 h-32 sm:w-40 sm:h-40 rounded-[2.5rem] bg-slate-50 dark:bg-slate-800/50 border-2 border-dashed border-primary/30 flex items-center justify-center overflow-hidden relative group">
                    {selectedImage ? <img src={selectedImage} className="w-full h-full object-cover group-hover:scale-105 transition-transform" alt="Preview" /> : <ImagePlus size={36} className="text-primary/40 group-hover:scale-110 transition-transform" />}
                  ...
                  <div className="w-full p-4 sm:p-5 bg-emerald-50 dark:bg-success/10 rounded-2xl sm:rounded-3xl border border-emerald-100 dark:border-success/20 text-center">
                    ...
                    <div className="flex items-center justify-center gap-2 text-emerald-600 dark:text-success"><CreditCard size={18} /><span className="font-mono text-[11px] sm:text-[13px] font-bold tracking-widest">A1:B2:C3:D4</span></div>
                  </div>
                </div>
                <div className="space-y-4">
                  <div className="relative"><IdCard size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" /><input type="text" placeholder="Cédula / DNI" required className={`${inputStyle} cursor-text`} /></div>
                  <div className="grid grid-cols-2 gap-4">
                    <div className="relative"><User size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" /><input type="text" placeholder="Nombre" required className={`${inputStyle} cursor-text`} /></div>
                    <div className="relative"><User size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" /><input type="text" placeholder="Apellido" required className={`${inputStyle} cursor-text`} /></div>
                  </div>
                  <div className="relative"><Mail size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" /><input type="email" placeholder="Correo electrónico" required className={`${inputStyle} cursor-text`} /></div>
                  <div className="relative"><Phone size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" /><input type="text" placeholder="Teléfono" required className={`${inputStyle} cursor-text`} /></div>
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                    <div className="relative"><select required className={inputStyle}><option value="" className={optionClassName}>Cargo...</option>{cargos.map(c => <option key={c} value={c} className={optionClassName}>{c}</option>)}</select></div>
                    <div className="relative"><select required className={inputStyle}><option value="" className={optionClassName}>Horario...</option>{horarios.map(h => <option key={h} value={h} className={optionClassName}>{h}</option>)}</select></div>
                  </div>
                </div>
                <div className="col-span-1 md:col-span-2 flex flex-col sm:flex-row gap-3 sm:gap-4 mt-6 pt-6 border-t border-slate-100 dark:border-slate-800">
                  ...
                  <button type="submit" className="w-full sm:flex-[2] py-3.5 sm:py-4 bg-primary hover:bg-indigo-600 text-white rounded-xl sm:rounded-2xl font-bold uppercase text-[10px] sm:text-[13px] tracking-widest shadow-lg shadow-primary/30 transition-colors">Confirmar Registro</button>
                </div>
              </form>
            </motion.div>
          </div>
