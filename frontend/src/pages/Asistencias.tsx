...
import type { Asistencia, Empleado } from "../services/api";
  ...
  <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div className="lg:col-span-2 space-y-6">
      ...
      <motion.div
        ...
        className={`${cardStyle} animate-pulse`}
      >
        ...
        <div className="space-y-4">
          {[1, 2, 3, 4, 5].map((i) => (
            <div
              key={i}
              className="w-full h-20 bg-slate-50 dark:bg-slate-800/50 rounded-2xl flex items-center px-4 gap-4"
            >
    ...
    <div className="lg:col-span-1">
      <motion.div
        ...
        className={`${cardStyle} h-full flex flex-col animate-pulse`}
      >
        ...
        <div className="space-y-8">
          {[1, 2, 3, 4].map((i) => (
            <div
              key={i}
              className="pl-6 border-l-2 border-slate-100 dark:border-slate-800 relative"
            >
    ...
    try {
      ...
      const dataOrdenada = dataAsistencias.sort(
        (a: Asistencia, b: Asistencia) => {
          ...
          const timeB = new Date(b.marcaSalida || b.marcaEntrada).getTime();
          return timeB - timeA;
        },
      );
  ...
  const historialFiltrado = useMemo(() => {
    return historialCompleto.filter((item) => {
      ...
      const matchBusqueda = busquedaGlobal
        ? textoBusqueda.includes(busquedaGlobal.toLowerCase())
        : true;
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <motion.div {...animProps} className={cardStyle}>
              <div className="flex items-center gap-3 mb-6 pb-6 border-b border-slate-100 dark:border-slate-800">
                ...
                <h2 className="font-black uppercase italic tracking-tighter dark:text-white text-2xl">
                  Filtros de Búsqueda
                </h2>
              </div>
                          ...
                          >
                            ...
                            <td className="p-4 text-center">
                              <span className="inline-flex items-center gap-1.5 text-[11px] font-mono font-bold text-slate-600 dark:text-slate-400 uppercase">
                                ...
                                />{" "}
                                {item.empleado.nfcUid}
                              </span>
                            </td>
                            <td className="p-4 text-right">
                              <span
                                className={`inline-flex items-center justify-center gap-1.5 px-4 py-2 rounded-full text-[9px] font-black uppercase tracking-wider ${item.estadoEntrada === "A_TIEMPO" ? "text-emerald-600 bg-emerald-50 dark:text-success dark:bg-success/10" : item.estadoEntrada === "TARDE" ? "text-amber-600 bg-amber-50 dark:text-warning dark:bg-warning/10" : "text-red-500 bg-red-50 dark:text-danger dark:bg-danger/10"}`}
                              >
                                {item.estadoEntrada === "A_TIEMPO" ? (
                                  <CheckCircle2 size={14} />
                                ) : item.estadoEntrada === "TARDE" ? (
                                  <AlertCircle size={14} />
                                ) : (
                                  <XCircle size={14} />
                                )}
                          ...
                          <td colSpan={6} className="p-8 text-center">
                            <div className="flex flex-col items-center justify-center text-slate-400 dark:text-slate-500">
                              ...
                              <p className="text-[11px] mt-1 font-medium">
                                Intenta cambiar la fecha o el empleado
                              </p>
                            </div>
                          </td>
                        </tr>
            ...
            >
              ...
              <div className="flex-1 space-y-6">
                <AnimatePresence>
                  {enVivo.length > 0 ? (
                    enVivo.map((lectura) => {
                      const esSalida = lectura.marcaSalida !== null;

                      const puntoTimelineClass = esSalida
                        ? "bg-amber-500 dark:bg-warning"
                        : "bg-blue-500 dark:bg-blue-400";

                      const pillEstadoClass =
                        lectura.estadoEntrada === "A_TIEMPO"
                          ? "bg-emerald-50 dark:bg-success/10 text-emerald-600 dark:text-success"
                          : lectura.estadoEntrada === "TARDE"
                            ? "bg-amber-50 dark:bg-warning/10 text-amber-600 dark:text-warning"
                            : "bg-red-50 dark:bg-danger/10 text-red-600 dark:text-danger";
                      ...
                      return (
                        <motion.div
                          ...
                          className="relative pl-7 border-l-2 border-slate-100 dark:border-slate-800"
                        >
                          <span
                            className={`absolute -left-[11px] top-1 w-5 h-5 rounded-full border-[5px] border-white dark:border-slate-900 ${puntoTimelineClass}`}
                          ></span>
                          ...
                          <div className="bg-slate-50 dark:bg-slate-800/50 p-5 rounded-2xl hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors border border-transparent dark:border-slate-800/50 hover:border-slate-200 dark:hover:border-slate-700 cursor-default">
                            <div className="flex justify-between items-start mb-3">
                              <p className="text-sm font-black text-slate-800 dark:text-white uppercase tracking-tight line-clamp-1">
                                {lectura.empleado.nombre}{" "}
                                {lectura.empleado.apellido}
                              </p>
                              <span
                                className={`text-[10px] font-black italic px-2 py-0.5 rounded-md shrink-0 ml-2 ${esSalida ? "bg-amber-100 text-amber-700 dark:bg-warning/20 dark:text-warning" : "bg-blue-100 text-blue-700 dark:bg-blue-900/30 dark:text-blue-400"}`}
                              >
                                {esSalida
                                  ? formatearHora(lectura.marcaSalida)
                                  : formatearHora(lectura.marcaEntrada)}
                              </span>
                            </div>
                            ...
                            <div className="flex justify-between items-end mt-1">
                              ...
                              <span
                                className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 rounded-lg text-[9px] font-black uppercase tracking-wider ${pillEstadoClass}`}
                              >
                                {esSalida ? "Salida" : "Entrada"} -{" "}
                                {formatearEstado(lectura.estadoEntrada)}
                              </span>
                            </div>
                          </div>
                        </motion.div>
