...
// Adaptado al Layout Oscuro (bg-slate-900)
...
const SkeletonHorario = () => (
  <div className={`${cardStyle} animate-pulse flex flex-col h-full`}>
    <div className="absolute top-6 right-6 w-14 h-4 bg-slate-200 dark:bg-slate-800 rounded-full"></div>
    ...
    <div className="flex items-start gap-4 mb-6 pt-1">
      ...
      <div className="w-full space-y-2 pt-1">
        ...
        <div className="w-2/3 h-3 bg-slate-200 dark:bg-slate-800 rounded mt-2"></div>
      </div>
    </div>
    
    <div className="space-y-3 mb-6 pt-4 border-t border-slate-50 dark:border-slate-800/50 flex-1">
    ...
    <div className="flex gap-2 w-full pt-4 border-t border-slate-50 dark:border-slate-800/50 mt-auto">
      ...
      <div className="h-10 w-12 bg-slate-200 dark:bg-slate-800 rounded-xl shrink-0"></div>
    </div>
  </div>
);
      ...
      <main className="pb-8 max-w-[1600px] mx-auto">
        ...
        <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-8">
          ...
          <button 
            ...
            className="bg-primary hover:bg-indigo-600 text-white px-6 py-3 rounded-2xl font-bold uppercase text-[11px] tracking-widest shadow-lg shadow-primary/20 flex items-center justify-center gap-2 transition-colors whitespace-nowrap self-start sm:self-end w-full sm:w-auto"
          >
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {isLoading ? (
            ...
            <div className="col-span-full py-12 text-center">
              ...
              <p className="text-xs font-medium text-slate-500 dark:text-slate-500">Ajusta los filtros o crea uno nuevo</p>
            </div>
          ) : (
            <AnimatePresence mode="popLayout">
              {filteredHorarios.map((horario) => (
                <motion.div
                  ...
                  layout
                  {...animProps}
                  className={`${cardStyle} group flex flex-col h-full ${actionHoverEffect}`}
                >
                  <div className={`absolute top-6 right-6 px-3 py-1 rounded-full text-[8px] font-bold uppercase ${horario.status === 'Activo' ? 'bg-emerald-50 text-emerald-600 dark:bg-success/10 dark:text-success' : 'bg-red-50 text-red-600 dark:bg-danger/10 dark:text-danger'}`}>
                    ● {horario.status}
                  </div>
                  ...
                  <div className="flex items-start gap-4 mb-5 pt-1">
                    ...
                    <div className="min-w-0 pr-14 pt-1">
                      ...
                      <p className="text-[11px] font-medium text-slate-500 dark:text-slate-400 mb-2 truncate">{horario.tipo}</p>
                  ...
                  <div className="space-y-3 pt-4 border-t border-slate-50 dark:border-slate-800/50 flex-1 mb-5">
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5 font-medium"><CalendarDays size={14}/> Días Laborables</span>
                      <span className="font-semibold text-slate-700 dark:text-slate-200 truncate max-w-[130px] text-right">{formatDias(horario.dias)}</span>
                    </div>
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5 font-medium"><Users size={14}/> Empleados</span>
                      <span className="font-semibold text-slate-700 dark:text-slate-200">{horario.empleados} asignados</span>
                    </div>
                    <div className="flex items-center justify-between text-xs">
                      <span className="text-slate-500 dark:text-slate-400 flex items-center gap-1.5 font-medium"><Watch size={14}/> Tolerancia</span>
                  ...
                  <div className="flex gap-2 pt-4 border-t border-slate-50 dark:border-slate-800/50 mt-auto">
                    <button 
                      ...
                      className="flex-1 flex justify-center items-center gap-2 bg-slate-50 dark:bg-slate-800/50 text-slate-600 dark:text-slate-300 py-2.5 rounded-xl text-[10px] font-bold uppercase hover:bg-primary dark:hover:bg-primary hover:text-white dark:hover:text-white transition-all duration-150 border border-transparent hover:border-primary/20"
                    >
                      <Edit size={14} /> <span className="italic">Editar</span>
                    </button>
                    <button 
                      onClick={() => handleDelete(horario.id, horario.nombre)}
                      className="p-2.5 bg-red-50 dark:bg-danger/10 text-red-500 dark:text-danger rounded-xl hover:bg-red-500 dark:hover:bg-danger hover:text-white transition-all border border-transparent hover:border-red-200"
                    >
                      <Trash2 size={16} />
                    </button>
                  </div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-slate-900/60 z-[200] flex justify-center items-center p-4 backdrop-blur-sm">
            ...
            >
              ...
              <p className="text-[9px] sm:text-[10px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-8">Definición de turnos corporativos</p>
              ...
              <form className="space-y-5" onSubmit={(e) => { e.preventDefault(); setIsModalOpen(false); }}>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="relative">
                    ...
                    <div className="relative">
                      <Sunrise size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" />
                  ...
                  <div className="relative">
                    ...
                    <div className="relative">
                      <Moon size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" />
                      <input type="time" required className={inputStyle} />
                    </div>
                  </div>
                </div>
                ...
                <div className="relative">
                  ...
                  <div className="relative">
                    <Watch size={16} className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400 dark:text-slate-500" />
                    <input 
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-6">
                  ...
                  <div className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-2xl flex flex-col items-center justify-center border border-slate-100 dark:border-slate-800">
                    ...
                    <span className="text-[8px] font-semibold uppercase text-slate-500 dark:text-slate-400 mt-1">Empleados Asignados</span>
                  </div>
                </div>
