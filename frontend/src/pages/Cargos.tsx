...
// Importamos la API que creaste (¡Asegúrate de que la ruta sea correcta!)
...
export default function Cargos() {
  ...
  // Ahora Cargos usa 'any' o puedes crear una Interface que coincida con Spring Boot
  ...
  const handleOpenModal = (mode: "add" | "edit", cargo?: Cargo) => {
    ...
    if (mode === "edit" && cargo) {
      setFormData({
        id: cargo.id || 0, // <-- Agregamos "|| 0"
        nombre: cargo.nombre,
        descripcion: cargo.descripcion,
        activo: cargo.activo ?? true, // <-- Agregamos "?? true"
      });
    } else {
  ...
  // --- CREAR O EDITAR EN BASE DE DATOS ---
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      fetchDatos(); // Recargar la tabla con los datos frescos de la BD

      Swal.fire({
        title:
          modalMode === "add" ? "¡Cargo Registrado!" : "¡Cargo Actualizado!",
        text: `El cargo de ${formData.nombre} ha sido guardado en la base de datos.`,
        icon: "success",
        background: isDark ? "#0f172a" : "#fff",
        color: isDark ? "#f8fafc" : "#334155",
        customClass: {
          popup:
            "rounded-[2rem] border border-transparent dark:border-slate-800",
        },
      });
    } catch {
  ...
  // (Por ahora usaremos el botón de desactivar para la misma función que el de eliminar,
  // ya que nuestro backend está programado con Soft Delete).
  // --- ACTIVAR / DESACTIVAR EN BASE DE DATOS ---
  const handleToggleStatus = (cargo: Cargo) => {
    const isDark = document.documentElement.classList.contains("dark");
    const isActivating = !cargo.activo; // Si está en false, lo queremos activar
    ...
    Swal.fire({
      ...
      if (result.isConfirmed) {
        try {
          if (isActivating) {
            // Para reactivar, mandamos un "actualizar" con activo en true
            await api.actualizarCargo(cargo.id!, { ...cargo, activo: true });
          } else {
            // Para desactivar, usamos tu ruta DELETE
      ...
      <main className="pb-8 max-w-[1600px] mx-auto">
        ...
        >
          ...
          <button
            ...
            className="bg-primary hover:bg-indigo-600 text-white px-7 py-3.5 rounded-2xl font-bold uppercase text-[13px] tracking-widest shadow-lg shadow-primary/20 flex items-center justify-center gap-2 transition-colors self-start sm:self-end whitespace-nowrap w-full sm:w-auto"
          >
        ...
        <motion.div
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          className="flex flex-wrap gap-3 sm:gap-4 bg-white dark:bg-slate-900 p-4 rounded-[2rem] border border-slate-100 dark:border-slate-800 items-center shadow-sm mb-8"
        >
          ...
          <div className="relative flex-1 w-full sm:w-auto min-w-[160px]">
            ...
            >
              ...
              <option value="Activo" className={optionClassName}>
                Activos
              </option>
              <option value="Inactivo" className={optionClassName}>
                Inactivos
              </option>
            </select>
          </div>
          ...
          {isLoading ? (
            ...
            <div className="col-span-full py-12 text-center">
              ...
              <p className="text-sm font-medium text-slate-500 dark:text-slate-500">
                Ajusta los filtros de búsqueda o crea uno nuevo
              </p>
            </div>
          ) : (
            <AnimatePresence mode="popLayout">
              {filteredCargos.map((cargo, index) => (
                <motion.div
                  ...
                  className={`${cardStyle} flex flex-col group ${actionHoverEffect} ${!cargo.activo ? "opacity-75 hover:opacity-100 grayscale-[0.3]" : ""}`}
                >
                  <div
                    className={`absolute top-6 right-6 px-3 py-1.5 rounded-full text-[9px] font-bold uppercase ${cargo.activo ? "bg-emerald-50 text-emerald-600 dark:bg-success/10 dark:text-success" : "bg-red-50 text-red-600 dark:bg-danger/10 dark:text-danger"}`}
                  >
                  ...
                  <div className="flex items-center gap-4 mb-5 pt-1">
                    ...
                    <div className="min-w-0 pr-12">
                      <h3 className="text-[17px] font-bold text-slate-800 dark:text-white leading-tight truncate">
                        {cargo.nombre}
                      </h3>
                      <div className="flex items-center gap-1.5 text-[11px] font-semibold text-primary uppercase mt-1">
                        <Users size={14} /> {cargo.empleadosCount || 0}{" "}
                        Empleados
                      </div>
                    </div>
                  </div>
                  ...
                  <div className="flex gap-2 w-full pt-4 border-t border-slate-50 dark:border-slate-800/50 mt-auto">
                    ...
                    >
                      <Edit size={14} />{" "}
                      <span className="italic truncate">Editar</span>
                    </button>

                    {cargo.activo ? (
                      <button
                        onClick={() => handleToggleStatus(cargo)}
                        className="flex-1 flex justify-center items-center gap-1.5 py-3 rounded-xl text-[11px] font-bold uppercase transition-all border border-transparent bg-amber-50 dark:bg-warning/10 text-amber-600 dark:text-warning hover:bg-amber-500 hover:text-white dark:hover:bg-warning"
                      >
                        <PowerOff size={14} />{" "}
                        <span className="italic truncate">Desactivar</span>
                      </button>
                    ) : (
                      <button
                        onClick={() => handleToggleStatus(cargo)}
                        className="flex-1 flex justify-center items-center gap-1.5 py-3 rounded-xl text-[11px] font-bold uppercase transition-all border border-transparent bg-emerald-50 dark:bg-success/10 text-emerald-600 dark:text-success hover:bg-emerald-500 hover:text-white dark:hover:bg-success"
                      >
                        <CheckCircle2 size={14} />{" "}
                        <span className="italic truncate">Activar</span>
                      </button>
                    )}
                  </div>
                </motion.div>
        ...
        {isModalOpen && (
          <div className="fixed inset-0 bg-slate-900/60 z-[200] flex justify-center items-center p-4 backdrop-blur-sm">
            <motion.div
              ...
              className="bg-white dark:bg-slate-900 p-6 sm:p-8 rounded-[3rem] sm:rounded-[3.5rem] w-full max-w-[600px] relative shadow-2xl border border-transparent dark:border-slate-800 max-h-[90vh] overflow-y-auto"
            >
              <button
                onClick={() => setIsModalOpen(false)}
                className="absolute right-6 top-6 sm:right-8 sm:top-8 text-3xl text-slate-400 hover:text-red-500 transition-colors"
              >
                &times;
              </button>
              <h2 className="text-2xl sm:text-3xl font-bold uppercase italic tracking-tighter mb-1 text-slate-800 dark:text-white">
                {modalMode === "add" ? "Crear Nuevo" : "Actualizar"}{" "}
                <span className="text-primary not-italic">Cargo</span>
              </h2>
              <p className="text-[10px] sm:text-[11px] font-medium text-slate-500 dark:text-slate-400 uppercase tracking-widest mb-8">
                Definición de responsabilidades y jerarquía
              </p>

              <form className="space-y-6" onSubmit={handleSubmit}>
                <div className="relative">
                  <label className="text-[10px] font-bold uppercase text-primary mb-2 block ml-2">
                    Nombre del Cargo
                  </label>
                  <div className="relative">
                    <Briefcase
                ...
                <div className="flex flex-col sm:flex-row gap-3 sm:gap-4 mt-8 pt-6 border-t border-slate-100 dark:border-slate-800">
                  <button
                    type="button"
                    onClick={() => setIsModalOpen(false)}
                    className="w-full sm:flex-1 py-4 rounded-xl sm:rounded-2xl font-bold uppercase text-[11px] text-slate-500 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 hover:bg-red-50 hover:text-red-500 dark:hover:bg-danger/10 dark:hover:text-danger transition-all border border-transparent hover:border-red-200 dark:hover:border-danger/20"
                  >
                    Cancelar
                  </button>
                  <button
                    type="submit"
                    className="w-full sm:flex-[2] py-4 bg-primary hover:bg-indigo-600 text-white rounded-xl sm:rounded-2xl font-bold uppercase text-[12px] tracking-widest shadow-lg shadow-primary/30 transition-colors"
                  >
                    {modalMode === "add"
                      ? "Confirmar Registro"
                      : "Guardar Cambios"}
